...
# def residual_mlp(scope: Scope, signal: Signal, hidden_dim=4):
#     """
...
def residual_mlp(scope: Scope, signal: Signal, hidden_dim=4, output_dim=2):
    ...
    # 第一层: (1, hidden_dim)
    W1 = scope.param('W1', nn.initializers.glorot_uniform(), (1, hidden_dim))
    b1 = scope.param('b1', nn.initializers.zeros, (hidden_dim,))
    # 第二层: (hidden_dim, output_dim) 这里 output_dim=2
    W2 = scope.param('W2', nn.initializers.glorot_uniform(), (hidden_dim, output_dim))
    b2 = scope.param('b2', nn.initializers.zeros, (output_dim,))
