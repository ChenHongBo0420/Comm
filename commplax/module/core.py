...
def fdbp(
    ...
    """
    自适应 DBP：在每一步补偿中对相位补偿的缩放因子 gamma 进行自适应更新，
    使用归一化误差指标，并对 gamma 进行范围剪切，以减少误差累积、稳定更新，
    实现在 DBP 架构上的较大性能提升。
    """
    x, t = signal
    # 初始化自适应相位缩放因子 gamma
    ...
    for i in range(steps):
        ...
        # 计算归一化误差：用相对功率变化作为误差指标
        power_before = jnp.mean(jnp.square(jnp.abs(x)))
        power_after = jnp.mean(jnp.square(jnp.abs(x_new)))
        error = (power_after - power_before) / (power_before + eps)
        
        # 更新 gamma 使用 LMS 规则（注意误差的正负可能需要调试）
        gamma = gamma - mu * error
        # 限制 gamma 的范围，防止过大或过小
        gamma = jnp.clip(gamma, 0.5, 1.5)
