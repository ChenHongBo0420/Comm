...
def fdbp(
    ...
    for i in range(steps):
        ...
        alpha = jax.nn.sigmoid(bridge_alpha)  # scalar
        x_fused = alpha * x_ignore + (1.0 - alpha) * x_ixpm

        # 如果两路 t 不同，需要先做对齐或报错，这里简单假设 t_ignore == t_ixpm
        # 你可以加 assert 或插值操作
        t_fused = t_ignore  # or t_ixpm, 如果一致
