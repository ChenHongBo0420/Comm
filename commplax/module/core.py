...
def fdbp(
    ...
    hidden_dim=2,
    use_alpha=True,
    mu=0.0001  # 学习率，用于 LMS 更新 gamma 和 alpha
):
    """
    保持原 fdbp(D->N)结构:
      1) 色散补偿 (D)
      2) 非线性补偿 (N) —— 在此基础上增加自适应相位补偿因子 gamma，
         并对 residual 缩放因子 alpha 采用 LMS 更新机制
    ...
    # 如果使用 alpha，则定义初始值（可以用 scope.param 定义初始值，后续手动更新）
    if use_alpha:
        alpha = scope.param('res_alpha', nn.initializers.zeros, ())
    else:
        alpha = 1.0
    ...
    for i in range(steps):
        ...
        # --- (B) 非线性补偿 (N) 使用 gamma 进行相位校正
        c, tN = scope.child(mimoconv1d, name=f'NConv_{i}')(
            Signal(jnp.abs(x)**2, td),
            taps=ntaps,
            kernel_init=n_init
        )
        # 应用相位补偿：使用手动 LMS 更新的 gamma
        ...
        error_gamma = power_after - power_before
        gamma = gamma - mu * error_gamma

        # --- (C) residual MLP：对 |x_nonlinear|^2 做 MLP 得到 residual
        res_val, t_res = scope.child(residual_mlp, name=f'ResCNN_{i}')(
            Signal(jnp.abs(x_nonlinear)**2, tN),
            hidden_dim=hidden_dim
        )
        res_val_cplx = jnp.asarray(res_val, x_nonlinear.dtype)
        res_val_cplx_2d = res_val_cplx[:, None]    # 调整为 (N,1)

        # --- 手动 LMS 更新 alpha
        # 计算 residual 加入前后的信号功率
        power_before_res = jnp.mean(jnp.square(jnp.abs(x_nonlinear)))
        x_updated = x_nonlinear + alpha * res_val_cplx_2d
        power_after_res = jnp.mean(jnp.square(jnp.abs(x_updated)))
        error_alpha = power_after_res - power_before_res
        alpha = alpha - mu * error_alpha

        # 更新信号和时间：将更新后的信号传递到下一步
        x, t = x_updated, t_res
