...
def separate_gating_output(x):
    gate_real = jax.nn.sigmoid(x.real)
    gate_imag = jax.nn.sigmoid(x.imag)
...
def conv1d(
    ...
    x = conv_fn(x, h, mode=mode)
    x = separate_gating_output(x)  # Apply separate gating to the output
